...
sub handle_explicit_cursor_attributes
{
    ...
    # Collect declared explicit cursor names: lines like "<name> CURSOR FOR ..." in DECLARE sections
    my %cursor_names = ();
    while ($str =~ /(DECLARE.*?BEGIN)/isg) {
        my $declare_block = $&;
        while ($declare_block =~ /\b([A-Z0-9_\$]+)\s+CURSOR\b/igs) {
            my $cname = $1;
            $cursor_names{$cname} = 1;
        }
    }

    # If no declare block or no explicit cursors, nothing to do
    return $str if (!%cursor_names);

    # Prepare declarations for is_open boolean and a single reusable row count variable
    my @isopen_decls = map { "  ${_}_is_open BOOLEAN := FALSE;" } sort keys %cursor_names;
    my $rowcount_decl = '  v_ora2pg_row_count INTEGER;';

    # Inject declarations into DECLARE; if no DECLARE exists, create one before BEGIN
    my $injected = 0;
    if ($str =~ s/\b(DECLARE(?:\s+|\%ORA2PG_COMMENT\d+\%))/$1 . join("\n", @isopen_decls, $rowcount_decl) . "\n"/eis) {
        $injected = 1;
    } elsif ($str =~ s/\b(BEGIN(?:\s+|\%ORA2PG_COMMENT\d+\%))/'DECLARE\n' . join("\n", @isopen_decls, $rowcount_decl) . "\n" . $1/eis) {
        $injected = 1;
    }

    # After OPEN <cursor>; insert <cursor>_is_open := TRUE;
    foreach my $cname (sort { length($b) <=> length($a) } keys %cursor_names) {
        $str =~ s/(\bOPEN\s+$cname\s*;)/$1\n$cname\_is_open := TRUE;/igs;
    }

    # After CLOSE <cursor>; insert <cursor>_is_open := FALSE;
    foreach my $cname (sort { length($b) <=> length($a) } keys %cursor_names) {
        $str =~ s/(\bCLOSE\s+$cname\s*;)/$1\n$cname\_is_open := FALSE;/igs;
    }

    # Replace <cursor>%ISOPEN with <cursor>_is_open
    foreach my $cname (sort { length($b) <=> length($a) } keys %cursor_names) {
        $str =~ s/\b$cname\s*\%\s*ISOPEN\b/${cname}_is_open/igs;
    }

    # After FETCH <cursor> ..., insert GET DIAGNOSTICS v_ora2pg_row_count = ROW_COUNT;
    foreach my $cname (sort { length($b) <=> length($a) } keys %cursor_names) {
        $str =~ s/(\bFETCH\b[^;]*?\b$cname\b[^;]*;)/$1\nGET DIAGNOSTICS v_ora2pg_row_count = ROW_COUNT;/igs;
    }

    # Replace <cursor>%ROWCOUNT with v_ora2pg_row_count
    foreach my $cname (sort { length($b) <=> length($a) } keys %cursor_names) {
