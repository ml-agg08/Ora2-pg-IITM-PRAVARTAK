...
/;
	...
	foreach my $n (keys %{$self->{global_variables}})
	{
		...
		# Generate COALESCE pattern with default value if available
		my $coalesce_pattern = $self->_generate_coalesce_pattern($n);
		$i = 0;
		while ($fct_detail{code} =~ s/([^']+)\b$n\b([^']+)/$1$coalesce_pattern$2/is) { last if ($i++ > 100); };
		$i = 0;
		...
		$i = 0;
		while ($fct_detail{code} =~ s/([^\.']+)\b$self->{global_variables}{$n}{name}\b([^']+)/$1$coalesce_pattern$2/is) { last if ($i++ > 100); };

		# Replace global variable in DECLARE section too
		$i = 0;
		while ($fct_detail{declare} =~ s/([^']+)\b$n\b([^']+)/$1$coalesce_pattern$2/is) { last if ($i++ > 100); };
		$i = 0;
		while ($fct_detail{declare} =~ s/([^\.']+)\b$self->{global_variables}{$n}{name}\b([^']+)/$1$coalesce_pattern$2/is) { last if ($i++ > 100); };
	}
