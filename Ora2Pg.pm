...
sub _init
{
	...
	$self->{tables_last_scn} = ();
	if ($self->{incremental_mode} && !$self->{is_mysql} && !$self->{is_mssql})
	{
		my $dirprefix = '';
		$dirprefix = "$self->{output_dir}/" if ($self->{output_dir});
		my $scnfile = "${dirprefix}TABLES_SCN.log";
		if (-e $scnfile && open(my $sfh, '<', $scnfile))
		{
			while (my $l = <$sfh>)
			{
				chomp($l);
				...
				next if (!$l || ($l =~ /^\s*\#/));
				my ($t, $scn) = split(/:/, $l, 2);
				next if (!defined $t || !defined $scn);
				$scn =~ s/^\s+//;
				$scn =~ s/\s+$//;
				$self->{tables_last_scn}{lc($t)} = $scn if ($scn =~ /^\d+$/);
			}
			close($sfh);
		}
	}
...
sub _get_sql_statements
{
	...
	elsif (($self->{type} eq 'INSERT') || ($self->{type} eq 'COPY'))
	{
		...
		foreach my $table (@ordered_tables)
		{
			...
			if ($self->{cdc_ready} || $self->{incremental_mode})
			{
				my $sth = $self->{dbh}->prepare("SELECT CURRENT_SCN FROM v\$database") or $self->logit("FATAL: " . $self->{dbh}->errstr . "\n", 0, 1);
				$sth->execute or $self->logit("FATAL: " . $self->{dbh}->errstr . "\n", 0, 1);
				my @row = $sth->fetchrow();
...
sub _dump_table
{
	...
	if ($self->{pg_dsn} && !$has_geometry)
	{
		if ($self->{type} ne 'COPY')
		{
			...
			$s_out =~ s/,$//;
			$s_out .= ")";
			if ($self->{incremental_mode} && $#pg_colnames_pkey >= 0)
			{
				my %_pk = map { $_ => 1 } @pg_colnames_pkey;
				my @set_cols = grep { !$_pk{$_} } @{$self->{tables}{$table}{dest_column_name}};
				if ($#set_cols >= 0) {
					$s_out .= " ON CONFLICT (" . join(',', @pg_colnames_pkey) . ") DO UPDATE SET " . join(',', map { "$_ = EXCLUDED.$_" } @set_cols);
				} else {
					$s_out .= " ON CONFLICT (" . join(',', @pg_colnames_pkey) . ") DO NOTHING";
				}
			}
			elsif ($self->{insert_on_conflict}) {
...
sub _dump_fdw_table
{
	...
	# Build INSERT statement
	{
		...
		{
			my %_pk = map { $_ => 1 } @pg_colnames_pkey;
			my @all_cols = split(/,/, $col_list);
			my @set_cols = grep { !$_pk{$_} } @all_cols;
			if ($#set_cols >= 0) {
				$s_out .= " ON CONFLICT (" . join(',', @pg_colnames_pkey) . ") DO UPDATE SET " . join(',', map { "$_ = EXCLUDED.$_" } @set_cols);
			} else {
				$s_out .= " ON CONFLICT (" . join(',', @pg_colnames_pkey) . ") DO NOTHING";
...
END;
	...
	if ($self->{incremental_mode} && !$self->{is_mysql} && !$self->{is_mssql})
	{
		my $last_scn = $self->{tables_last_scn}{lc($table)};
		if (defined $last_scn && $last_scn =~ /^\d+$/)
		{
			($str =~ / WHERE /) ? $str .= ' AND ' : $str .= ' WHERE ';
			$str .= "ORA_ROWSCN > $last_scn";
			$self->logit("\tApplying INCREMENTAL_MODE filter on table: ORA_ROWSCN > $last_scn\n", 1);
		}
	}
...
sub _convert_package
{
	...
	if ($self->{packages}{$pkg}{desc} =~ /CREATE OR REPLACE PACKAGE\s+([^\s]+)(?:\s*\%ORA2PG_COMMENT\d+\%)*\s*((?:AS|IS)(?:\s*\%ORA2PG_COMMENT\d+\%)*)\s*(.*)/is)
	{
		...
		# Process package spec to extract public routines and global variables
		# First, collect public function/procedure names from the SPEC
		my $pub_decl = $glob_declare;
		$self->_remove_comments(\$pub_decl);
		$self->{package_public}{lc($pname)} ||= {};
		while ($pub_decl =~ /\b(PROCEDURE|FUNCTION)\s+([^\s\(\)]+)/ig) {
			my $n = $2;
			$n =~ s/^.*\.//;
			$n =~ s/"//g;
			next if (!$n);
			$self->{package_public}{lc($pname)}{lc($n)} = 1;
		}

		# Then, continue existing processing of the SPEC to extract globals
...
\$body\$ LANGUAGE plpgsql SECURITY DEFINER;
	...
	if ($fct_detail{code})
	{
		...
		# Generate REVOKE only for private routines inside packages.
		# Public routines (declared in SPEC) get no REVOKE; non-package routines get no REVOKE.
		if ($pname) {
			my $pkg_lc = lc($pname);
			my $fn_lc = lc($fct_detail{name});
			if (!exists $self->{package_public}{$pkg_lc}{$fn_lc}) {
				$revoke = "REVOKE ALL ON $type $name $fct_detail{args} FROM PUBLIC;\n";
				$revoke .= "REVOKE ALL ON $type $name$at_suffix $fct_detail{args} FROM PUBLIC;\n" if ($at_suffix);
			}
		}
...
sub _dump_to_pg
{
	...
	elsif (!$sprep)
	{
		$sql_out = '';
		my $conflict_clause = '';
		if ($self->{incremental_mode} && exists $self->{tables}{$table}{pg_colnames_pkey} && $#{$self->{tables}{$table}{pg_colnames_pkey}} >= 0)
		{
			my @pk = @{$self->{tables}{$table}{pg_colnames_pkey}};
			my %_pk = map { $_ => 1 } @pk;
			my @set_cols = grep { !$_pk{$_} } @{$self->{tables}{$table}{dest_column_name}};
			if ($#set_cols >= 0) {
				$conflict_clause = " ON CONFLICT (" . join(',', @pk) . ") DO UPDATE SET " . join(',', map { "$_ = EXCLUDED.$_" } @set_cols);
			} else {
				$conflict_clause = " ON CONFLICT (" . join(',', @pk) . ") DO NOTHING";
			}
		}
		elsif ($self->{insert_on_conflict}) {
		...
		foreach my $row (@$rows)
		{
			$sql_out .= $s_out;
			$sql_out .= join(',', @$row) . ")";
			$sql_out .= $conflict_clause if ($conflict_clause);
			$sql_out .= ";\n";
		}
	}
...
sub _generate_coalesce_pattern
{
	my ($self, $var_name) = @_;
	
	# Check if we have a default value for this variable
	if (exists $self->{global_variables}{$var_name}{default} && 
	    defined $self->{global_variables}{$var_name}{default} && 
	    $self->{global_variables}{$var_name}{default} ne '')
	{
		my $default_value = $self->{global_variables}{$var_name}{default};
		my $var_type = $self->{global_variables}{$var_name}{type} || 'text';
		
		# Handle different data types for proper PostgreSQL casting
		my $coalesce_pattern;
		if ($var_type =~ /^(integer|int|bigint|smallint|numeric|decimal|real|double|float)$/i)
		{
			# For numeric types, ensure the default value is properly quoted
			$default_value =~ s/^'//;
			$default_value =~ s/'$//;
			$coalesce_pattern = "COALESCE(current_setting('$var_name', true), '$default_value')::" . lc($var_type);
		}
		elsif ($var_type =~ /^(boolean|bool)$/i)
		{
			# For boolean types, handle Oracle TRUE/FALSE to PostgreSQL true/false
			$default_value =~ s/^'//;
			$default_value =~ s/'$//;
			$default_value = lc($default_value);
			$coalesce_pattern = "COALESCE(current_setting('$var_name', true), '$default_value')::boolean";
		}
		elsif ($var_type =~ /^(varchar|char|text|string)$/i)
		{
			# For string types, ensure proper quoting
			$default_value =~ s/^'//;
			$default_value =~ s/'$//;
			$coalesce_pattern = "COALESCE(current_setting('$var_name', true), '$default_value')";
		}
		elsif ($var_type =~ /^(date|timestamp|time)$/i)
		{
			# For date/time types, ensure proper quoting
			$default_value =~ s/^'//;
			$default_value =~ s/'$//;
			$coalesce_pattern = "COALESCE(current_setting('$var_name', true), '$default_value')::" . lc($var_type);
		}
		...
		return $coalesce_pattern;
	}
	else
	{
		# No default value available, fall back to the original pattern
		my $var_type = $self->{global_variables}{$var_name}{type} || 'text';
		return "current_setting('$var_name')::" . lc($var_type);
	}
}
